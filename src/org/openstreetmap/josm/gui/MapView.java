 ...
 *  addLayer:
 *      no playheadmarker
 *      layer is always inserted at position 0
 *      no audio
 *      no repaint
 *  removeLayer:
 *      no audio
 ...
 * @author imi
 */
public class MapView extends NavigatableComponent implements CanvasPresenter, PropertyChangeListener { //, PreferenceChangedListener {

    public MapView(CanvasView view) {
        super(view);
    }
    ...
    public void repaint() {
      ...
      Bounds box = getLatLonBounds(new Rectangle(0, 0, view.getWidth(), view.getHeight()));
      for (int i=0; i<visibleLayers.size(); i++) {
          visibleLayers.get(i).paint(g, this, box);
      }
      for (MapViewPaintable mvp : temporaryLayers) {
          mvp.paint(g, this, box);
      }
    }
